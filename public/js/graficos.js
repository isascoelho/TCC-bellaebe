document.addEventListener("DOMContentLoaded", function () {
  ...
  function filtrarPeriodoAnterior(lista, periodo) {
    ...
    let atualInicio = new Date(inicioHoje);
    let anteriorInicio = new Date(inicioHoje);
    let anteriorFim = new Date(inicioHoje);

    if (periodo === "sem") {
      atualInicio = obterInicioSemana(inicioHoje);
      ...
      atualInicio = new Date(inicioHoje.getFullYear(), inicioHoje.getMonth(), 1);
      ...
      atualInicio = new Date(inicioHoje.getFullYear(), inicioHoje.getMonth() - 2, 1);
      ...
      atualInicio = new Date(inicioHoje.getFullYear(), inicioHoje.getMonth() - 5, 1);
      ...
      atualInicio = new Date(inicioHoje.getFullYear(), inicioHoje.getMonth() - 11, 1);
      ...
      atualInicio = new Date(inicioHoje.getFullYear(), 0, 1);
    ...
    lista.forEach(item => {
      ...
      if (!mapa.has(chave)) {
        mapa.set(chave, {
          ...
          label: `Sem ${inicioSemana.toLocaleDateString("pt-BR", {
            day: "2-digit",
            month: "2-digit"
          })}`,
          total: 0
  ...
  function obterIconeResumo(tipo) {
    const icones = {
      ...
      despesa: `
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
          <path d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14m1-7V4.5a.5.5 0 0 0-1 0V8a.5.5 0 0 0 .146.354l2 2a.5.5 0 0 0 .708-.708z"/>
        </svg>
      `,
      percentual: `
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
          <path d="M13.442 2.558a1.5 1.5 0 0 1 0 2.121l-8.763 8.763a1.5 1.5 0 1 1-2.121-2.12l8.763-8.764a1.5 1.5 0 0 1 2.12 0"/>
          <path d="M5.5 6a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3m5 7a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3"/>
        </svg>
      `,
      saldo: `
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
          <path d="M0 3a2 2 0 0 1 2-2h12a1 1 0 0 1 1 1v10a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2zm3 1a1 1 0 0 0-1 1v7a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1z"/>
          <path d="M8 5a1.5 1.5 0 1 0 0 3a1.5 1.5 0 0 0 0-3"/>
        </svg>
      `,
      tendencia: `
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
  ...
  function abrirModalDetalhesDespesas(listaDespesas, labels, valores) {
    if (
      !modalDetalhesEl ||
      !detalhesResumoTopoEl ||
      !listaCategoriasDespesasEl ||
      !listaLancamentosDespesasEl
    ) {
      return;
    }
    ...
    detalhesResumoTopoEl.innerHTML = `
      ...
      <div class="detalhes-mini-card">
        ...
        <strong>${formatarMoeda(maiorValor)}</strong>
      </div>
    `;

    if (!labels.length) {
      listaCategoriasDespesasEl.innerHTML = `<p class="detalhe-vazio">Não há categorias de despesas no período.</p>`;
    } else {
      listaCategoriasDespesasEl.innerHTML = labels.map((label, index) => {
    ...
    if (!listaDespesas.length) {
      listaLancamentosDespesasEl.innerHTML = `<p class="detalhe-vazio">Não há lançamentos de despesas no período.</p>`;
    } else {
      const ordenadas = [...listaDespesas].sort((a, b) => {
        const dataA = obterData(a)?.getTime() || 0;
        const dataB = obterData(b)?.getTime() || 0;
        return dataB - dataA;
      });

      listaLancamentosDespesasEl.innerHTML = ordenadas.map(item => {
        const titulo = item.descricao || item.categoria || "Despesa";
        const categoria = item.categoria || "Sem categoria";
        const data = item.periodo || item.data || item.createdAt;

        return `
          <div class="detalhe-lancamento-item">
            <div>
              <div class="detalhe-lancamento-titulo">${titulo}</div>
              <span class="detalhe-lancamento-meta">${categoria} • ${formatarDataBR(data)}</span>
            </div>
...
const centerTextPlugin = {
  ...
  afterDraw(chart) {
    if (
      chart.canvas.id !== "graficoDespesasCategoria" ||
  ...
  callbacks: {
    label: context => {
      const semDespesas = context.chart.data.labels?.[0] === "Sem despesas";
      if (semDespesas) {
          ...
          callbacks: {
            label: context => {
              if (modoReceitasAtual === "percentual") {
                return `${formatarNumero(context.raw)}%`;
              }
  ...
  function atualizarResumoInteligente(receitas, despesas, resultado, receitasCat, despesasCat) {
    ...
    const maiorReceitaIndex = receitasCat.valores.length
      ? receitasCat.valores.indexOf(Math.max(...receitasCat.valores))
      : -1;

    const maiorDespesaIndex = despesasCat.valores.length
      ? despesasCat.valores.indexOf(Math.max(...despesasCat.valores))
      : -1;

    if (maiorReceitaIndex >= 0 && receitas > 0) {
      itens.push({
        iconSvg: obterIconeResumo("receita"),
        iconClass: "icon-receita",
    ...
    if (maiorDespesaIndex >= 0 && despesas > 0) {
      itens.push({
        iconSvg: obterIconeResumo("despesa"),
        iconClass: "icon-despesa",
    ...
    itens.push({
      iconSvg: obterIconeResumo("percentual"),
      iconClass: "icon-lucro",
    ...
    itens.push({
      iconSvg: obterIconeResumo("tendencia"),
      iconClass: "icon-tendencia",
    ...
    resumoInteligenteEl.innerHTML = itens.map(item => `
      <div class="resumo-item-v2">
        <div class="resumo-icon-v2 ${item.iconClass}">
          ${item.iconSvg}
        </div>
        <div>
  ...
  function atualizarTela() {
    ...
    const granularidadeEscolhida = granularidadeSelect?.value || "mensal";
    const granularidadeFinal = resolverGranularidade(granularidadeEscolhida);
    ...
    if (variacaoReceitasEl) {
      variacaoReceitasEl.textContent = calcularVariacao(totalReceitas, somar(receitasAnterior));
    }

    if (variacaoDespesasEl) {
      variacaoDespesasEl.textContent = calcularVariacao(totalDespesas, somar(despesasAnterior));
    }
    ...
    const semMovimento = totalReceitas === 0 && totalDespesas === 0;
    if (semDadosEl) {
      semDadosEl.style.display = semMovimento ? "block" : "none";
    }

    const receitasParaEvolucao = granularidadeFinal === "mensal"
      ? filtrarAnoAtual(estado.receitas)
      : receitasPeriodo;

    const despesasParaEvolucao = granularidadeFinal === "mensal"
      ? filtrarAnoAtual(estado.despesas)
      : despesasPeriodo;

    const serieTemporal = montarSerieComparativa(
      receitasParaEvolucao,
      despesasParaEvolucao,
      granularidadeFinal
    );
    ...
    graficoDespesasCategoria.data.labels = despesasCat.labels.length
      ? despesasCat.labels
      : ["Sem despesas"];
    graficoDespesasCategoria.data.datasets[0].data = despesasCat.valores.length
      ? despesasCat.valores
      : [1];
    graficoDespesasCategoria.data.datasets[0].backgroundColor = despesasCat.valores.length
      ? CORES_DESPESAS
      : ["#d9d9d9"];
    ...
    if (modoReceitasAtual === "percentual") {
      const totalCategoriasReceita = receitasCat.valores.reduce((a, b) => a + b, 0);
      const percentuais = totalCategoriasReceita > 0
        ? receitasCat.valores.map(valor => Number(((valor / totalCategoriasReceita) * 100).toFixed(2)))
        : [];

      graficoReceitasCategoria.data.labels = receitasCat.labels.length
        ? receitasCat.labels
        : ["Sem receitas"];
      graficoReceitasCategoria.data.datasets[0].data = receitasCat.labels.length
        ? percentuais
        : [0];

      if (totalReceitasRodapeEl) {
        totalReceitasRodapeEl.textContent = "100%";
      }
    } else {
      graficoReceitasCategoria.data.labels = receitasCat.labels.length
        ? receitasCat.labels
        : ["Sem receitas"];
      graficoReceitasCategoria.data.datasets[0].data = receitasCat.valores.length
        ? receitasCat.valores
        : [0];

      if (totalReceitasRodapeEl) {
        totalReceitasRodapeEl.textContent = formatarMoeda(totalReceitas);
      }
    }

    graficoReceitasCategoria.update();

    const quantidadePontos = serieTemporal.labels.filter(label => label !== "Sem dados").length || 1;
    const mediaReceitas = quantidadePontos ? serieTemporal.receitas.reduce((a, b) => a + b, 0) / quantidadePontos : 0;
    const mediaDespesas = quantidadePontos ? serieTemporal.despesas.reduce((a, b) => a + b, 0) / quantidadePontos : 0;

    if (rodapeEvolucaoEl) {
      const textoBase = granularidadeFinal === "semanal"
        ? "Receitas médias semanais"
        : "Receitas médias mensais";

      const textoDespesa = granularidadeFinal === "semanal"
        ? "Despesas médias semanais"
        : "Despesas médias mensais";

      rodapeEvolucaoEl.textContent =
        `${textoBase}: ${formatarMoeda(mediaReceitas)} • ${textoDespesa}: ${formatarMoeda(mediaDespesas)}`;
    }
  ...
  function carregarDados() {
    Promise.allSettled([
      fetch("/receitas", { credentials: "include" }).then(r => {
        if (!r.ok) throw new Error("Erro ao buscar receitas");
        return r.json();
      }),
      fetch("/despesas", { credentials: "include" }).then(r => {
        if (!r.ok) throw new Error("Erro ao buscar despesas");
        return r.json();
      })
    ])
      .then(([receitasResult, despesasResult]) => {
        estado.receitas = receitasResult.status === "fulfilled"
          ? receitasResult.value || []
          : [];

        estado.despesas = despesasResult.status === "fulfilled"
          ? despesasResult.value || []
          : [];
  ...
  btnVerDetalhesDespesas?.addEventListener("click", () => {
    ...
    abrirModalDetalhesDespesas(
      despesasFiltradasAtual,
      despesasCat.labels,
      despesasCat.valores
    );
  });
